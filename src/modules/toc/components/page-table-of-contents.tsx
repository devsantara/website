import { ListIcon } from 'lucide-react';
import * as React from 'react';

import type { TableOfContents as TableOfContentsData } from '#/modules/markdown/markdown.types';
import { normalizeEntries } from '#/modules/toc/toc.utils';
import { Button } from '#/ui/components/core/button';
import {
  Drawer,
  DrawerClose,
  DrawerContent,
  DrawerFooter,
  DrawerHeader,
  DrawerTitle,
  DrawerTrigger,
} from '#/ui/components/core/drawer';
import {
  Sheet,
  SheetContent,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
} from '#/ui/components/core/sheet';
import { cn } from '#/ui/utils';

import { TableOfContents } from './table-of-contents';

/**
 * Responsive shell placed once per article, adapting the same "On this page"
 * list to three widths:
 *
 * - **Desktop** (`xl` and up): fixed in the gutter just right of the centered
 *   article, always visible.
 * - **Tablet** (`md`–`xl`, where that gutter collapses): a floating button
 *   opening the list in a right-side {@link Sheet}.
 * - **Mobile** (below `md`): a full-width bar pinned to the very bottom like a
 *   bottom navigation bar, opening the list in a bottom {@link Drawer}.
 *
 * Renders nothing when the article has no h2/h3 headings.
 */
export function PageTableOfContents({ toc }: { toc: TableOfContentsData }) {
  const hasEntries = React.useMemo(() => normalizeEntries(toc).length > 0, [toc]);
  if (!hasEntries) return null;

  return (
    <>
      <aside className="fixed top-8 left-[calc(50%+23rem)] hidden max-h-[calc(100vh-4rem)] w-56 flex-col xl:flex">
        <p className="text-muted-foreground mb-3 flex shrink-0 items-center gap-2 text-sm">
          <ListIcon className="size-4" aria-hidden />
          On this page
        </p>
        <div className="toc-scroll min-h-0 overflow-y-auto">
          <TableOfContents toc={toc} />
        </div>
      </aside>

      <div className="hidden md:block xl:hidden">
        <TabletTableOfContents toc={toc} />
      </div>

      <div className="md:hidden">
        <MobileTableOfContents toc={toc} />
      </div>
    </>
  );
}

/**
 * Tablet tier: a floating outline button in the bottom-right corner opening the
 * TOC in a sheet that slides in from the right, echoing the desktop gutter's
 * position. Closes as soon as a link scrolls the page.
 */
function TabletTableOfContents({ toc }: { toc: TableOfContentsData }) {
  const [open, setOpen] = React.useState(false);

  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger
        className={cn('fixed right-4 bottom-6 z-40 shadow-md')}
        render={
          <Button variant="secondary">
            <ListIcon aria-hidden /> On this page
          </Button>
        }
      />
      <SheetContent side="right" className="w-80">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            <ListIcon className="text-muted-foreground size-4" aria-hidden />
            On this page
          </SheetTitle>
        </SheetHeader>
        <div className="toc-scroll min-h-0 flex-1 overflow-y-auto px-4 pb-4">
          <TableOfContents toc={toc} onNavigate={() => setOpen(false)} />
        </div>
      </SheetContent>
    </Sheet>
  );
}

/**
 * Mobile tier: a full-width bar fixed to the very bottom — styled like a mobile
 * bottom navigation bar, with a safe-area inset so it clears the home indicator
 * — opening the TOC in a bottom drawer the reader can swipe down to dismiss.
 * Closes as soon as a link scrolls the page.
 */
function MobileTableOfContents({ toc }: { toc: TableOfContentsData }) {
  const [open, setOpen] = React.useState(false);

  return (
    <Drawer open={open} onOpenChange={setOpen} showSwipeHandle>
      <DrawerTrigger className="bg-background/90 text-foreground supports-backdrop-filter:bg-background/75 fixed inset-x-0 bottom-0 z-40 flex items-center justify-center gap-2 border-t px-4 pt-3.5 pb-[max(0.875rem,env(safe-area-inset-bottom))] text-sm font-medium shadow-[0_-1px_3px_rgb(0_0_0/0.04)] backdrop-blur">
        <ListIcon className="text-muted-foreground size-4" aria-hidden />
        On this page
      </DrawerTrigger>
      <DrawerContent className="[--drawer-content-max-height:70dvh]">
        <DrawerHeader>
          <DrawerTitle className="flex items-center justify-center gap-2 text-base">
            <ListIcon className="text-muted-foreground size-4" aria-hidden />
            On this page
          </DrawerTitle>
        </DrawerHeader>
        <div className="toc-scroll mt-4 min-h-0 flex-1 overflow-y-auto px-4 pb-4">
          <TableOfContents toc={toc} onNavigate={() => setOpen(false)} />
        </div>
        <DrawerFooter className="border-t pt-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <DrawerClose render={<Button variant="secondary" />}>Close</DrawerClose>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
  );
}
